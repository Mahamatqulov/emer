    ...
    >
      <nav
        className={`mx-auto max-w-7xl transition-all duration-500 border  ${
          scrolled
            ? "rounded-[28px]  dark:bg-slate-900/50 shadow-lg backdrop-blur-xs dark:border-white/10"
            : "rounded-[28px] border-transparent bg-transparent shadow-none backdrop-blur-0"
        }`}
      >
        <div
          ...
          <div className="hidden lg:flex items-center gap-7">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`text-[13.5px] font-medium transition-colors hover:text-primary ${
                  scrolled
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.nav
              ...
              className={`lg:hidden border-t px-5 pb-5 pt-3 overflow-hidden ${
                scrolled
                  ? "border-black/5 dark:border-white/10"
                  : "border-white/10 bg-slate-900/80 backdrop-blur-xl rounded-b-[28px]"
              }`}
            >
              {navItems.map((item) => (
                <Link
